...
fn parse_string(input: Span) -> IResult<Span, SmallVec<[CobStringSegment; 1]>>
{
    // Opening "
    ...
    let (remaining, (last_segment_input, mut segments)) = fold_many0(
        parse_fragment,
        || (remaining, SmallVec::from_elem(CobStringSegment::default(), 1)),
        |(mut input, mut segments): (_, SmallVec<[CobStringSegment; 1]>), fragment| {
            match fragment {
...
impl From<&str> for CobStringSegment
{
...
impl From<char> for CobStringSegment
{
...
impl From<String> for CobStringSegment
{
    /// Note that `Self::write_to` -> `Self::from` is lossy because String has no awareness of
    /// multi-line string formatting. The string contents are preserved, but not their presentation in COB.
...
pub struct CobString
{
    ...
    pub segments: SmallVec<[CobStringSegment; 1]>,
...
impl CobString
{
    ...
    pub fn try_parse(fill: CobFill, content: Span) -> Result<(Option<Self>, CobFill, Span), SpanError>
    {
        ...
        if segments.len() == 0 {
            segments.push(CobStringSegment::default());
        }
...
impl From<char> for CobString
{
    fn from(character: char) -> Self
    {
        Self {
            fill: CobFill::default(),
            segments: SmallVec::from_elem(CobStringSegment::from(character), 1),
            cached: None,
        }
    }
}

impl From<&str> for CobString
{
    fn from(string: &str) -> Self
    {
        Self {
            fill: CobFill::default(),
            segments: SmallVec::from_elem(CobStringSegment::from(string), 1),
            cached: None,
        }
    }
}

impl From<String> for CobString
{
