...
/// Trait representing special [`UiInstructions`](UiInstruction) that can be loaded with [`StyleSheet`].
///
...
impl<T: CobwebStyle> UiInstruction for StyleLoader<T>
{
    fn apply(self, rc: &mut ReactCommands, node: Entity)
    {
        ...
        let token = rc.on(resource_mutation::<StyleSheet>(),
            move
            |
        ...
        //todo: invoke loader in build finalization
    }
}
