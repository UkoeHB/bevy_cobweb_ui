...
use sickle_ui_scaffold::theme::icons::Icons;

use super::entity_component_list::{EntityComponentList, EntityComponentListPlugin, UiEntityComponentListExt};
use crate::widgets::layout::column::UiColumnExt;
use crate::widgets::layout::foldable::{Foldable, UiFoldableExt};
use crate::widgets::layout::panel::UiPanelExt;
use crate::widgets::layout::row::UiRowExt;
use crate::widgets::layout::scroll_view::UiScrollViewExt;
use crate::widgets::layout::sized_zone::{SizedZoneConfig, UiSizedZoneExt};
use crate::widgets::menus::menu_item::{MenuItem, MenuItemConfig, UiMenuItemExt};
...
)
{
    for hierarchy in &q_hierarchies {
        let Ok(mut component_list) = q_entity_component_list.get_mut(hierarchy.component_list) else {
            continue;
        };
...
impl HierarchyContainer
{
    pub fn foldable_theme() -> Theme<Foldable>
    {
        ...
        let selected_theme =
            PseudoTheme::deferred(vec![PseudoState::Selected], HierarchyContainer::selected_style);
    ...
    fn hierarchy_for(&mut self, root_entity: Entity) -> UiBuilder<Entity>
    {
        self.column(|column| {
            ...
            let main_zone = column
                .sized_zone(SizedZoneConfig { size: 70., min_size: 200. }, |zone| {
                    ...
                    zone.panel("Hierarchy content".into(), |panel| {
                        ...
                        panel.scroll_view(None, |scroll_view| {
                            let node_container = scroll_view
                                .column(|_| {})
                                .insert(HierarchyNodeContainer { hierarchy: hierarchy_id })
                                .id();
            ...
            column.sized_zone(SizedZoneConfig { size: 25., ..default() }, |zone| {
            ...
            column.commands().ui_builder(main_zone).insert((
                ...
                HierarchyContainer { root: root_entity, selected: None, component_list },
            ));
        })
    }
}
