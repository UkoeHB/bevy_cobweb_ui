...
impl SpecData
{
    ...
    fn clear_cached_edits(&mut self)
    {
        self.params.retain(|_, (saved, temp)| {
                *temp = Value::Null;
                ...
                if let (&Value::Null, &Value::Null) = (saved, temp) { return false; }
                true
            }
        });
    ...
    fn extract_specs(&mut self, file: &LoadableFile, spec_invocation: &str, map: &mut Map<String, Value>, merge_params: bool) -> bool
    {
        for (key, value) in map.iter_mut() {
            if key == SPEC_CONTENT_SYMBOL {
                ...
                match self.params.contains(key) {
                    true => {
                        let (mut saved_param, mut temp_param) = self.params.get_mut(key).unwrap();
                        match merge_params {
                            true => {
                    ...
                    false => {
                        //todo: check that newly-added params are present in the content after insertions are applied
                        // so users can be warned if they made a typo or forgot to use a param/have extra params
                        match merge_params {
                            true => {
                ...
                self.inserts_cache.push((key.clone(), value.take()));
            } else if key.starts_with(COMMENT_KEYWORD) {
                continue;
            }
            else {
                tracing::warn!("ignoring invalid spec key {} in spec invocation {} in file {}",
        ...
        match value {
            Value::Object(map) => {
                ...
                for (key, value) in map.iter_mut() {
                    ...
                    if key.starts_with(SPEC_INSERTION_MARKER) {
                        if let Some((to_insert)) = self.inserts_cache.iter().position(|(cached_key, _)| cached_key == key) else {
                ...
                for insert_idx in insertion_cache.drain(..) {
                    ...
                    let Object::Map(insertion_map) = value else {
            ...
            Value::Array(arr) => {
                ...
                for value in arr.iter_mut() {
                    ...
                    let Some((to_insert)) = self.inserts_cache.iter().position(|(cached_key, _)| cached_key == value_str) else {
                        continue;
                    };
                    insertion_cache.push(to_insert);
                }
                ...
                for insert_idx in insertion_cache.drain(..) {
                    ...
                    let vec_idx = arr.iter().position(|v| {
    ...
    fn recursively_resolve_spec_content(&self, file: &LoadableFile, value: &mut Value, is_resolving_insertion: bool)
    {
        match value {
            Value::Object(map) => {
                let mut insertion_cache = SmallVec::<[usize; 4]>::default();
                ...
                for insert_idx in insertion_cache.drain(..) {
                    ...
                    for (key, value) in insertion_map.iter() {
                        ...
                        self.recursively_resolve_spec_content(file, value, true);
                        map.insert(key.clone(), value.clone());
                    }
                }
            }
            Value::Array(arr) => {
                ...
                arr.retain_mut(|value| {
                    ...
                    if value_str.starts_with(SPEC_INSERTION_MARKER) {
                        index_head -= 1
                        if let Some((to_insert)) = self.inserts_cache.iter().position(|(cached_key, _)| cached_key == value_str) else {
                            if !is_resolving_insertion {
            ...
            Value::String(string) => {
                ...
                let Some((saved, temp)) = self.params.get(string) else {
    ...
    fn update_from_specs_override(&mut self, file: &LoadableFile, spec_invocation: &str, overrides: &mut Map<String, Value>)
    {
        // Extract override edits.
        self.extract_edits(file, spec_invocation, overrides, true);
    ...
    fn write_to_value(&mut self, file: &LoadableFile, spec_invocation: &str, value: &mut Value)
    {
        ...
        *value = self.content.clone();
        self.recursively_resolve_spec_content(value, false);
...
impl SpecsMap
{
    ...
    pub(crate) fn import_specs(&mut self, import_file: &LoadableFile, file: &LoadableFile, imported: &SpecsMap)
    {
        for (spec_key, data) in imported.map.iter() {
            if let Some(_) = self.map.insert(spec_key, data.clone()) {
    ...
    fn update_or_insert_spec(&mut self, file: &LoadableFile, key: &str, value: &mut Value) -> bool
    {
        match try_parse_spec_invocation(key) {
            Ok(None) => {
                // Insert a new spec.
                if let Some(_) = self.map.insert(key.into(), SpecData::new(file, key, value)) {
            ...
            Ok(Some((new_key, spec_key))) => {
                ...
                if new_key != spec_key {
                    ...
                    data.update_from_specs_override(file, key, value);
    ...
    /// Extracts a spec into a spec invocation (which may override parts of the spec).
    fn try_extract_spec_entry(&mut self, file: &LoadableFile, key: &str, value: &mut Value) -> bool
    {
        let spec_key = match try_parse_spec_invocation(key) {
            Ok(Some(spec_key)) => spec_key,
...
/// imports).
pub(crate) fn extract_specs_section(
    file: &LoadableFile,
...
)
{
    ...
    for (key, value) in data.iter_mut() {
        ...
        // Note: if recursion isn't possible due to invalid content, errors will be thoroughly logged in the data parser.
