...
) -> Option<ReflectedStyle>
{
    ...
    let Some(inherited) = style_entry.last() else {
...
)
{
    ...
    stylesheet.insert(
        &StyleRef { file: file.clone(), path: current_path.clone() },
        style_value,
        type_id,
        long_name,
    );
...
)
{
    ...
    for (shortname, initial_size) in stack_tracker.drain(..) {
        style_stack.get_mut(&shortname).unwrap().truncate(initial_size);
    }
...
pub(crate) fn parse_stylesheet_file(
    type_registry: &TypeRegistry,
    stylesheet: &mut StyleSheet,
    file: StyleFile,
    data: Value,
